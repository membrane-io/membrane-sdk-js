import { $$, SchemaTraversal } from "../lib";
...
describe("SchemaTraversal", () => {
  describe("constructed with type name", () => {
    ...
    it("accepts a type name from an imported schema by id", () => {
      const t1 = new SchemaTraversal(
        schema,
        "00000000-0000-1000-8000-000000000000:Thing"
      );
      expect(t1.getContext()).toEqual({
        isRoot: true,
        schema: schema.imports[0],
        type: (schema.imports[0].types as ImportType[]).find(
          (t) => t.name === "Thing"
        ),
        wrappers: [],
      });
  ...
  describe("constructed with typed", () => {
    ...
    it("accepts a plain type from an imported schema", () => {
      const t1 = new SchemaTraversal(schema, {
        type: "00000000-0000-1000-8000-000000000000:Thing",
      });
      expect(t1.getContext()).toEqual({
        isRoot: true,
        schema: schema.imports[0],
    ...
    it("accepts a list-wrapped type from an imported schema", () => {
      const t1 = new SchemaTraversal(schema, {
        type: "List",
        ofType: "00000000-0000-1000-8000-000000000000:Thing",
      });
      expect(t1.getContext()).toEqual({
        isRoot: true,
        schema: schema.imports[0],
  ...
  describe("enterMember", () => {
    it("requires the member name to be provided", () => {
      const t1 = new SchemaTraversal(schema);
      expect(() => t1.enterMember()).toThrow(
        "Expected member name to be a non-empty string"
      );
    });
    ...
    it("requires the args to be an object", () => {
      const t1 = new SchemaTraversal(schema);
      expect(() => t1.enterMember("field", "not an object")).toThrow(
        "Expected args to be an object"
      );
    });
  ...
  describe("getTyped", () => {
    ...
    it("computes the typed of a ref", () => {
      ...
      expect(t1.getTyped()).toEqual({
        type: "Ref",
        ofType: { type: "OwnThing" },
      });
    });
