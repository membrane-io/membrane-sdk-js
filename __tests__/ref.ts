...
describe("$$", () => {
  ...
  it("handles complex ref strings correctly", () => {
    ...
    expect(t1.getIn(["path", 1, "args", "y", "path", 0, "name"])).toEqual(
      "ref"
    );
    expect(
      t1.getIn(["path", 1, "args", "y", "path", 0, "args"])
    ).toEqualImmutable(I.Map({ with: "args" }));
    expect(t1.getIn(["path", 1, "args", "y", "path", 1, "name"])).toEqual(
      "and"
    );
    expect(
      t1.getIn(["path", 1, "args", "y", "path", 1, "args"])
    ).toEqualImmutable(I.Map());
    expect(t1.getIn(["path", 1, "args", "y", "path", 2, "name"])).toEqual(
      "stuff"
    );
    expect(
      t1.getIn(["path", 1, "args", "y", "path", 2, "args"])
    ).toEqualImmutable(I.Map());

    // arguments(...)
    expect(t1.getIn(["path", 2, "name"])).toEqual("arguments");
    expect(t1.getIn(["path", 2, "args"])).toEqualImmutable(
      I.Map({ value: true })
    );
  });
});
...
describe("PathElem", () => {
  ...
  it("can replace its arguments", () => {
    ...
    expect(t1.path.get(0).withArgs(args).args).toEqualImmutable(
      t2.path.get(2).args
    );
  });
});
